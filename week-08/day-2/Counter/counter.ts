...
- Create `Counter` class
  ...
  - there are two ways to create the class:
    ...
    - with 1 input parameter called `counter` which is set as the initial
  ...
  - and we can `reset()` the counter to the initial value set during the
    construction of the class
- use `export Counter` at the end of your file
- Check if everything is working fine with the proper test
  - Download `counter.test.ts` and place it next to your solution
  - Run `npm install tape @types/tape --save-dev` in the root of your repository
    folder (where you have a `package-lock.json` file already)
